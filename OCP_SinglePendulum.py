...
class OcpSinglePendulumWithNNCost(OcpSinglePendulum):
    ...
    def objective_cost(self,N,x,x_des,u):
        cost = 0
        running_costs = [None,]*(N)
        for i in range(N):
            """
            running cost function = 
            """
            running_costs[i] = self.w_x * (x[i,0] - x_des[0])**2 + 0*self.w_v * (x[i,1] - x_des[1])**2 
            if(i<N-1):
                running_costs[i] += self.w_u* u[i]*u[i]
            cost += running_costs[i] 
        cost += self.w_v*(x[N-1,1])**2
        #Terminal cost
    ...
    def OCP_setup(self, x_init, N, x_des, X_guess=None, U_guess=None):
        ...
        for i in range(1,N):
            self.opti.subject_to(x[i,0] == x[i-1,0] + self.dt * x[i-1,1])
            self.opti.subject_to(x[i,1] == x[i-1,1] + self.dt * (u[i-1] + self.g * ca.sin(x[i-1,0])))
